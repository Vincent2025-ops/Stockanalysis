...
interface Top10Stock { 
  ...
  date?: string;
  days?: number;     // 已連續符合天數
  summary?: string;  // 精簡核心資訊 (指標數值及已連續符合x日)
  rule?: string;     // 🎯 各策略標準買賣點規則 (如: KD<20買 / KD>80賣)
  marketDate?: string; // 🎯 今日大盤交易日 (如: 2026-09-24)
  isUpToDate?: boolean; // 🎯 是否已同步為今日最新
}
...
// 🎯 指標名稱轉換：統一映射為純英文名稱 (Volume Ratio)
...
// 🎯 文字濾網：自動替換雲端可能殘留的舊文字與舊規則
const cleanFeatureText = (text: string): string => {
  if (!text) return '';
  return text
    .replace(/目前指標數值/g, '指標數值')
    .replace(/【ChipRatio】/g, '【Volume Ratio】')
    .replace(/【成交量均量比策略（Volume Ratio）】/g, '【Volume Ratio】')
    .replace(/成交量均量比策略（Volume Ratio）/g, 'Volume Ratio')
    .replace(/成交量均量比/g, 'Volume Ratio')
    .replace(/ChipRatio/g, 'Volume Ratio')
    .replace(/籌碼集中度提升，顯示主力介入/g, '成交量顯著放大，顯示買盤積極介入')
    .replace(/10 日均量比值大於 0\.5 且三大法人買超 ； 小於 0\.5 時平倉/g, '10 日均量比值>1.5 買進 ； 跌破 10MA 或 均量比>2.0 且為實體長黑 K 平倉')
    .replace(/10 日均量比值大於 0\.5 且三大法人買超 ；小於 0\.5 時平倉/g, '10 日均量比值>1.5 買進 ； 跌破 10MA 或 均量比>2.0 且為實體長黑 K 平倉')
    .replace(/10 日均量比值大於 0\.5 且三大法人買超/g, '10 日均量比值>1.5 買進');
};
...
const App: React.FC = () => {
    ...
    const [cloudSyncDate, setCloudSyncDate] = useState<string>('');
    ...
    // 🎯 loadTop10：嚴格檢查雲端資料日期與今日交易日是否一致
    const loadTop10 = async () => {
      setLoadingMarket(true); setMarketMsg(''); setExpandedMarketRow(null);
      try { 
        const stocks = await window.go.main.App.FetchTop10FromCloud(); 
        setTop10Stocks(stocks);

        if (stocks && stocks.length > 0) {
          const syncDate = stocks[0].date || '未知日期';
          const marketDate = stocks[0].marketDate || '';
          const isLatest = stocks[0].isUpToDate;
          setCloudSyncDate(syncDate);

          if (isLatest) {
            setMarketMsg(`✅ 雲端資料已同步為今日最新！(資料日期: ${syncDate})`);
          } else {
            setMarketMsg(`⚠️ 雲端資料尚未更新至今日！(今日交易日: ${marketDate}，雲端目前資料為: ${syncDate})`);
          }
        } else {
          setMarketMsg(`⚠️ 雲端目前無任何符合條件的選股資料`);
        }
      } 
      catch (err: any) { setMarketMsg(`❌ 同步失敗: ${err}`); } finally { setLoadingMarket(false); }
    };
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-slate-900 text-slate-200 relative overflow-hidden">
                ...
                <div className="bg-slate-800 p-8 rounded-2xl shadow-2xl border border-slate-700 max-w-md w-full z-10 text-center">
                    <svg className="w-16 h-16 text-blue-500 mx-auto mb-6 animate-bounce" fill="none" stroke="currentColor" viewBox="0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <main className="flex-1 overflow-y-auto p-8 relative scroll-smooth w-full">
          ...
          {activeTab === 'market' && (
            <div className="w-full mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 relative z-10">
              <header className="mb-6 border-b border-slate-800/80 pb-6 flex justify-between items-end">
                <div>
                  <h1 className="text-4xl font-black flex items-center gap-3 text-white tracking-tight">
                    <Trophy className="text-cyan-400" size={32} /> 每日選股訊號池
                  </h1>
                  {cloudSyncDate && (
                    <div className="flex items-center gap-2 text-cyan-400 text-sm font-mono mt-2 font-bold animate-in fade-in">
                      <CalendarClock size={16} /> 雲端資料日期: 
                      <span className={`px-2.5 py-0.5 rounded-lg border font-mono ${
                        top10Stocks.length > 0 && top10Stocks[0].isUpToDate
                          ? 'text-emerald-400 bg-emerald-950/70 border-emerald-800/80'
                          : 'text-amber-400 bg-amber-950/70 border-amber-800/80'
                      }`}>
                        {cloudSyncDate} {top10Stocks.length > 0 && !top10Stocks[0].isUpToDate ? '(尚未更新)' : '(最新)'}
                      </span>
                    </div>
                  )}
                </div>
                <div className="flex gap-4">
                  <button onClick={loadTop10} disabled={loadingMarket} className="flex items-center gap-2 px-6 py-3 bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl font-bold disabled:opacity-50 transition-all shadow-lg shadow-cyan-900/20">
                    <Download size={20} /> 同步雲端數據
                  </button>
                  <button onClick={exportAllData} disabled={loadingMarket} className="flex items-center gap-2 px-6 py-3 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-xl border border-slate-700 disabled:opacity-50 transition-all">
                    <Save size={20} /> 匯出 CSV
                  </button>
                </div>
              </header>
              ...
              {/* 面板最佳風險報酬比（盈虧比）態樣提示欄 */}
              <div className="mb-6 p-4 rounded-2xl border bg-slate-900/80 border-cyan-500/30 flex items-start gap-3.5 backdrop-blur-md shadow-lg shadow-cyan-950/20">
                <div className="p-2 rounded-xl bg-cyan-950/60 border border-cyan-700/40 text-cyan-400 shrink-0 mt-0.5">
                  <Info size={20} />
                </div>
                <div className="text-sm leading-relaxed">
                  <div className="text-cyan-400 font-black text-base tracking-wide flex items-center gap-2 mb-1">
                    🎯 最佳風險報酬比（盈虧比）態樣指南
                    <span className="text-xs font-mono font-bold text-rose-400 bg-rose-950/60 border border-rose-800/60 px-2 py-0.5 rounded-md">
                      符合度越高（天數越少）排名越前
                    </span>
                  </div>
                  <p className="text-slate-300">
                    <strong className="text-white">【黃金買點：連續符合 1～2 日】</strong>：代表訊號剛被觸發、剛進入買點表態。此態樣進場成本最貼近主力、停損點明確好防守，<strong>承擔下檔風險最小且潛在向上空間最大，盈虧比最佳</strong>。<br />
                    <span className="text-slate-400">【鈍化風險：連續符合 &gt;5 日】</span>：代表訊號已延續多日，順勢股通常已拉開短線漲幅不宜盲追；逆勢超賣股則可能陷入低檔無量鈍化，切忌盲目摸底。
                  </p>
                </div>
              </div>
              ...
              {/* 訊息提示框：支援 ⚠️ 呈現黃色警告樣式 */}
              {marketMsg && (
                <div className={`mb-6 p-4 rounded-xl border flex items-center gap-3 text-lg font-medium backdrop-blur-md shadow-lg ${
                  marketMsg.includes('❌') 
                    ? 'bg-rose-900/20 border-rose-800/50 text-rose-400' 
                    : marketMsg.includes('⚠️')
              ...
              {/* 表格容器 */}
              <div className="bg-[#0B1121]/80 backdrop-blur-xl rounded-2xl overflow-x-auto border border-slate-800/80 shadow-2xl custom-scrollbar">
                <table className="w-full text-left min-w-[900px]">
                  <thead className="bg-slate-900/80 border-b border-slate-800">
                    <tr>
                      <th className="py-4 pl-6 pr-3 text-slate-400 uppercase text-xs md:text-sm font-bold tracking-wider w-[12%] whitespace-nowrap">股票代號</th>
                      <th className="py-4 px-3 text-slate-400 uppercase text-xs md:text-sm font-bold tracking-wider w-[13%] whitespace-nowrap">名稱</th>
                      <th className="py-4 px-3 text-slate-400 uppercase text-xs md:text-sm font-bold tracking-wider w-[12%] whitespace-nowrap">最新收盤價</th>
                      <th className="py-4 px-3 text-slate-400 uppercase text-xs md:text-sm font-bold tracking-wider whitespace-nowrap">即時指標核心數值與符合天數 (點擊展開明細)</th>
                      <th className="py-4 pr-6 pl-3 text-slate-400 uppercase text-xs md:text-sm font-bold tracking-wider w-[90px] text-center whitespace-nowrap">操作</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-800/50">
                    ...
                    {Object.entries(groupedStocks).map(([indicatorName, stocks]) => {
                      const strategyRule = stocks[0]?.rule || "";
                      const displayIndicatorName = formatIndicatorName(indicatorName);

                      return (
                        <React.Fragment key={indicatorName}>
                          {/* 標題純英文化：例如 Volume Ratio 爆發潛力池 */}
                          <tr className="bg-slate-800/40">
                            <td colSpan={5} className="px-6 py-3 border-l-4 border-cyan-500 bg-gradient-to-r from-cyan-900/25 to-transparent">
                              <div className="flex items-center justify-between w-full flex-wrap gap-2.5">
                                <div className="flex items-center gap-3">
                                  <span className="text-cyan-400 font-black tracking-widest text-sm">
                                    {displayIndicatorName} 爆發潛力池
                                  </span>
                                  {strategyRule && (
                                    <span className="text-xs font-mono font-bold text-amber-300 bg-amber-950/70 border border-amber-700/60 px-2.5 py-0.5 rounded-md flex items-center gap-1.5 shadow-sm">
                                      <Target size={12} className="text-amber-400" />
                                      買賣規則: {strategyRule}
                                    </span>
                                  )}
                                </div>
                                <span className="text-xs font-mono text-cyan-300/90 font-bold bg-cyan-950/80 border border-cyan-800/70 px-2.5 py-0.5 rounded shrink-0">
                                  連續 1~2 日優先列於前 (盈虧比最佳)
                                </span>
                              </div>
                            </td>
                          ...
                          {stocks.map((s, i) => {
                            const rowKey = `${indicatorName}-${s.id}-${i}`;
                            const isExpanded = expandedMarketRow === rowKey;
                            const isPrimeEntry = s.days !== undefined && s.days <= 2;
                            const cleanSummary = cleanFeatureText(s.summary || s.feature);
                            ...
                            return (
                              <React.Fragment key={rowKey}>
                                <tr className="hover:bg-slate-800/40 transition-colors group">
                                  ...
                                  <td className="py-3.5 px-3 text-slate-200 font-bold text-base md:text-lg">{s.name}</td>
                                  <td className={`py-3.5 px-3 font-mono text-lg md:text-xl font-bold ${getRowPriceColor(s)}`}>{s.price}</td>
                                  
                                  {/* 🎯 核心資訊文字與徽章（方案 C：左側純文字，徽章與箭頭同組右靠） */}
                                  <td className="py-3.5 px-3">
                                    <div 
                                      onClick={() => setExpandedMarketRow(isExpanded ? null : rowKey)}
                                      className="flex items-center justify-between gap-4 cursor-pointer group/desc hover:text-cyan-300 transition-colors select-none w-full"
                                      title="點擊展開查看完整說明與技術條件"
                                    >
                                      {/* 左側：指標核心說明文字 */}
                                      <div className="flex items-center gap-2.5 min-w-0 pr-2">
                                        <span className="text-rose-500 font-bold shrink-0 text-base">💥</span>
                                        <span className="text-slate-200 text-sm md:text-base font-semibold group-hover/desc:text-cyan-200 tracking-wide">
                                          {cleanSummary}
                                        </span>
                                      </div>
                                      ...
                                      <div className="flex items-center gap-2.5 shrink-0">
                                        {isPrimeEntry && (
                                          <span className="px-2.5 py-0.5 bg-rose-950/80 text-rose-300 border border-rose-600/60 rounded-md text-xs font-bold whitespace-nowrap shadow-sm shrink-0 flex items-center gap-1">
                                            <span className="w-1.5 h-1.5 rounded-full bg-rose-500 inline-block animate-pulse"></span>
                                            最佳買點
                                          </span>
                                        )}
                                        <div className="p-1.5 rounded-lg bg-slate-800/90 border border-slate-700/80 group-hover/desc:border-cyan-500/60 group-hover/desc:bg-slate-700/80 shrink-0 transition-all">
                                          <ChevronDown size={15} className={`text-slate-400 transition-transform duration-300 ${isExpanded ? 'rotate-180 text-cyan-400' : 'group-hover/desc:text-cyan-400'}`} />
                                        </div>
                                      </div>
                                    </div>
                                  </td>
                                  ...
                                  <td className="py-3.5 pr-6 pl-3 text-center">
                                    <button onClick={() => handleAddToWatchlist(s)} className="text-xs px-3.5 py-1.5 bg-slate-800 border border-slate-700 hover:bg-purple-600 hover:border-purple-500 hover:text-white text-slate-300 rounded-xl transition-all inline-flex items-center gap-1.5 font-bold shadow-sm whitespace-nowrap shrink-0">
                                      <Plus size={14} /> 觀察
                                    </button>
                                  </td>
                                </tr>
                                ...
                                {/* 下拉表單：完整展開內容 */}
                                {isExpanded && (
                                  <tr className="bg-[#040812]/95 shadow-inner">
                                    <td colSpan={5} className="p-0 border-b border-slate-700/60">
                                      <div className="px-8 py-5 animate-in fade-in slide-in-from-top-2 duration-300">
                                        <div className="flex items-center justify-between mb-3 border-b border-slate-800 pb-2">
                                          <div className="flex items-center gap-3 text-cyan-400 font-bold text-sm tracking-wider">
                                            <span className="flex items-center gap-1.5"><Zap size={16} /> 【{displayIndicatorName}】即時選股特徵與完整條件診斷</span>
                                            {s.rule && (
                                              <span className="text-xs font-mono font-bold text-amber-300 bg-amber-950/70 border border-amber-700/50 px-2 py-0.5 rounded">
                                                買賣規則: {s.rule}
                                              </span>
                                            )}
                                          </div>
                                          {s.date && (
                                            <div className="text-xs font-mono text-slate-400 flex items-center gap-1.5">
                                              <CalendarClock size={13} /> 雲端同步日: {s.date}
                                            </div>
                                          )}
                                        </div>
                                        
                                        {/* 完整即時說明文字區塊 */}
                                        <div className="bg-[#0B132B] p-4 rounded-xl border border-slate-700/60 text-slate-200 text-sm md:text-base leading-relaxed font-sans mb-3 select-text shadow-inner">
                                          {cleanFeatureText(s.feature)}
                                        </div>

                                        {/* 條件解讀標籤 */}
                                        <div className="flex flex-wrap items-center gap-2 pt-1">
                                          <span className="text-xs text-slate-500 font-bold">技術型態診斷:</span>
                                          {isPrimeEntry && (
                                            <span className="px-2.5 py-1 bg-rose-950/70 text-rose-400 border border-rose-800/60 rounded-lg text-xs font-bold flex items-center gap-1">
                                              <Target size={13} /> 剛進買點 (連續 {s.days} 日，盈虧比最佳)
                                            </span>
                                          )}
                                          {s.feature.includes("符合") && (
                                            <span className="px-2.5 py-1 bg-emerald-950/70 text-emerald-400 border border-emerald-800/60 rounded-lg text-xs font-bold flex items-center gap-1">
                                              <ShieldCheck size={13} /> 符合進場條件
                                            </span>
                                          )}
                                          {s.feature.includes("買點") && (
                                            <span className="px-2.5 py-1 bg-amber-950/70 text-amber-400 border border-amber-800/60 rounded-lg text-xs font-bold flex items-center gap-1">
                                              <TrendingUp size={13} /> 近期關鍵買點
                                            </span>
                                          )}
                                        </div>
                                      </div>
