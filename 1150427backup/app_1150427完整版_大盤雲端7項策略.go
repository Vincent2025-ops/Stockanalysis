...
// parseFinancialNumber 將財經網頁中常見的數字格式字串
...
// cleanText 移除字串中的 HTML 標籤與多餘的空白
func cleanText(s string) string {
	...
	inTag := false
	for _, ch := range s {
		if ch == '<' { inTag = true } else if ch == '>' { inTag = false } else if !inTag { res.WriteRune(ch) }
	}
...
func extractCells(row string) []string {
	...
	for {
		...
		if endTag == -1 { endTag = strings.Index(row, "</th") }
		if endTag == -1 { break }
...
func resolveStock(input string) (resolvedID string, price float64, name string, err error) {
	...
	if errTWSE == nil {
		...
		if json.NewDecoder(respTWSE.Body).Decode(&result) == nil {
			if dataList, ok := result["data"].([]interface{}); ok { twseData = dataList }
		}
		respTWSE.Body.Close()
	}
	...
	if isNumeric && len(input) > 0 { return input, 0, "未知名稱", nil }
...
func (a *App) FetchStockAnalysis(input string) StockAnalysisResult {
	...
	if twsePE > 0 && price > 0 {
		...
		result.Basic.PE, result.Basic.EPS, result.Basic.ROE = "負值(虧損)", "虧損無PE", "虧損無PE"
	}
...
func (a *App) LoadWatchlist() []WatchlistItem {
	file, err := ioutil.ReadFile(getWatchlistFile())
...
func (a *App) SaveWatchlist(list []WatchlistItem) error {
	...
	if err != nil { return err }
	return ioutil.WriteFile(getWatchlistFile(), data, 0644)
}
