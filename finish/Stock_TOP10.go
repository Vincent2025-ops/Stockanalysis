		...
		Result []struct {
			Indicators struct {
				Quote []struct {
					...
					Volume []float64 `json:"volume"` // ✨ 僅此處加入 volume 接收歷史成交量
				} `json:"quote"`
			} `json:"indicators"`
		} `json:"result"`
	} `json:"chart"`
...
func scanBollingerBands(stocks []StockData) {
	...
	for i := 0; i < topCount; i++ {
		...
		if err == nil && len(yResp.Chart.Result) > 0 && len(yResp.Chart.Result[0].Indicators.Quote) > 0 {
			...
			// ✨ 計算 10 日均量比值 (ChipRatio = 當日量 / 10日平均量)
			...
			if len(validVols) >= 10 {
				last10 := validVols[len(validVols)-10:]
				sumV := 0.0
				...
				avgV := sumV / 10.0
				if avgV > 0 {
					ratio := float64(sortedByVol[i].Volume) / avgV
					for j := range stocks {
						if stocks[j].StockID == sid {
