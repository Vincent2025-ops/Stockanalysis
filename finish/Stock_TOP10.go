...
// 新增：第 7 大策略「布林通道下軌掃描」與「10日均量比值(ChipRatio)」，針對前 200 大熱門股進行歷史運算。
package main
...
type StockData struct {
	...
	ChipRatio   float64 // 籌碼集中度估算 (10日均量比值)
...
func fetchStockData() ([]StockData, error) {
	...
	for _, item := range dataList {
		...
		tempStocks = append(tempStocks, StockData{
			...
			PrevPrice: prevPrice,
			Volume:    volume,
			ChipRatio: -1.0,   // 初始化均量比值為 -1.0 (代表尚未計算，避免干擾排序)
...
// =====================================================================
// 3. 歷史指標專屬計算與回推邏輯 (布林通道 & 10日均量)
// =====================================================================

// scanHistoricalIndicators 找出前 200 大熱門股，並追溯歷史資料計算布林通道與 10 日均量
func scanHistoricalIndicators(stocks []StockData) {
	fmt.Println("📊 啟動歷史指標掃描：開始篩選市場前 200 大熱門股 (布林通道與10日均量)...")
	...
	// 建立熱門股清單與歷史價格、成交量 Map
	targetMap := make(map[string]bool)
	priceHistory := make(map[string][]float64)
	volHistory := make(map[string][]float64)
	...
	for i := 0; i < topCount; i++ {
		...
		volHistory[sid] = append(volHistory[sid], float64(sortedByVol[i].Volume))
	}

	// 2. 往前追溯 19 個交易日的歷史資料 (Bollinger 需要 20 天)
	daysNeeded := 19
	daysFound := 0
	targetDate := time.Now()
	...
	for attempts := 0; attempts < 35 && daysFound < daysNeeded; attempts++ {
		...
		if len(dataList) > 0 {
			...
			for _, item := range dataList {
				...
				// 只有前 200 大熱門股才記錄歷史價格與成交量
				if targetMap[stockID] {
					priceStr := strings.ReplaceAll(fmt.Sprintf("%v", row[8]), ",", "")
					volStr := strings.ReplaceAll(fmt.Sprintf("%v", row[2]), ",", "") // 成交股數
					
					p, errP := strconv.ParseFloat(priceStr, 64)
					v, errV := strconv.ParseFloat(volStr, 64)
					
					if errP == nil && p > 0 && errV == nil && v >= 0 {
						priceHistory[stockID] = append(priceHistory[stockID], p)
						volHistory[stockID] = append(volHistory[stockID], v)
					}
				}
			}
	...
	for i := range stocks {
		...
		if targetMap[sid] {
			...
			// --- B. 計算 10 日均量比值 (ChipRatio) ---
			// 公式: 當日成交量 / 近 10 日平均成交量
			if len(vols) >= 10 {
				sumVol := 0.0
				for j := 0; j < 10; j++ {
					sumVol += vols[j]
				...
				avgVol := sumVol / 10.0
				if avgVol > 0 {
					stocks[i].ChipRatio = vols[0] / avgVol
					countChipRatio++
				}
			}
		}
	}
	fmt.Printf("✅ 歷史指標分析完成！共成功計算 布林通道: %d 檔, 10日均量比值: %d 檔。\n", countBollinger, countChipRatio)
	
	if countBollinger == 0 || countChipRatio == 0 {
		fmt.Println("❌ 警告：指標計算數為 0，請確認是否觸發了 API 限制！")
	}
}
...
func getTop10(stocks []StockData, indicator string) []StockData {
	sort.Slice(stocks, func(i, j int) bool {
		...
		case "ChipRatio":
			return stocks[i].ChipRatio > stocks[j].ChipRatio // 均量比值越高代表今日越放量
		case "Bollinger":
...
func exportToCSV(fileName string, allTop10 map[string][]StockData) error {
	...
	for _, indicator := range order {
		...
		} else if indicator == "ChipRatio" {
			for _, s := range stocks {
				if s.ChipRatio >= 0 { // 有歷史資料計算出的都會 >= 0
					calculatedCount++
				}
			}
		} else {
		...
		for _, stock := range stocks {
			...
			case "ChipRatio":
				if stock.ChipRatio < 0 { // 跳過未計算的股票
					continue
				}
				valueStr = fmt.Sprintf("%.2f", stock.ChipRatio)
				// 📌 根據要求：原說明不動，補充完整策略規則，並串接目前指標數值
		...
		if !hasValidStock {
			msg := "今日無符合條件個股"
			// 🛡️ 防呆：如果根本沒算出來，要誠實地提示使用者
			if (indicator == "Bollinger" || indicator == "ChipRatio") && calculatedCount == 0 {
...
func main() {
	...
	// 2. 啟動布林通道與 10 日均量掃描 (運算需要約 40~60 秒)
	scanHistoricalIndicators(stocks)
