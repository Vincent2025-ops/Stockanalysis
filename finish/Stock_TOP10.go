...
func fetchStockData() ([]StockData, error) {
	req, err := http.NewRequest("GET", "https://www.twse.com.tw/exchangeReport/STOCK_DAY_ALL?response=json", nil)
	if err != nil {
		return nil, err
	}

	// 加上常見的瀏覽器 User-Agent 來降低被阻擋的機率
	...
	// 先檢查 HTTP 狀態碼是否為 200 OK
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("證交所 API 拒絕連線，HTTP 狀態碼: %d", resp.StatusCode)
	}
