...
// 🎯 核心修正 1：擴大支援特殊負號標記與中文「虧損」字眼，徹底防止負值誤判為正或 0
func parseFinancialNumber(s string) (float64, error) {
	s = strings.TrimSpace(s)
	
	// 判斷是否帶有負數特徵 (新增「虧」與「負」以防備中文財報格式)
	...
	for _, ch := range s {
		if (ch >= '0' && ch <= '9') || ch == '.' || ch == '-' {
			clean.WriteRune(ch)
		} else if ch == '－' || ch == '−' || ch == '—' || ch == '–' { // 擴大支援各種全形/特殊/破折號減號
			clean.WriteRune('-')
		}
	}
...
func (a *App) FetchNBreakout() ([]Top10Stock, error) {
	...
	// ==========================================
	// 🎯 核心升級：雙軌報價抓取與「終極備援」機制
	// ==========================================

	// TWSE 抓取 (升級為 OpenAPI 優先，徹底防護 Cloudflare 阻擋與假日維護)
	...
	// 🎯 檢核機制 1 升級 (終極備援)：
	// 如果證交所與櫃買的每日報價 API 「雙雙陣亡」(通常發生在週末大維護，回傳 HTML 導致解析失敗)
	// 我們直接把最前面抓到的「公司基本面名單 (companyInfoMap)」全部拿來當作候選名單，強迫進入 Yahoo K 線驗證！
	if len(candidates) == 0 {
		fmt.Println("[System] 報價 API 無回應或週末維護中，啟動終極備援：直接展開全市場名單掃描...")
		for id := range companyInfoMap {
			if len(id) == 4 && !strings.HasPrefix(id, "00") {
				candidates = append(candidates, stockCandidate{
					ID:         id,
					Name:       "台股 " + id,
					Price:      50,      // 給定虛擬價格通過初步排序
					Volume:     500000,  // 給定虛擬成交量
					TradeValue: 25000000, 
				})
			}
		}
	...
	for _, c := range candidates {
		...
		go func(cand stockCandidate) {
			...
			// 🎯 排除已噴出：目前收盤價不可超過起漲高點的 5%，嚴格限制在突破邊緣或仍在箱型內-> 1.05放寬至1.2
			if todayClose > ignitionHigh*1.2 { return }

			// 必須要有洗盤期 (至少表態後經過 2 天)
			if n - 1 - ignitionIdx < 2 { return }

			// 🎯 洗盤確認：表態日之後到昨天之間，必須出現過量縮至表態量 50% 以下的窒息量->放寬至90%
			washed := false
			for i := ignitionIdx + 1; i < n-1; i++ {
				if vols[i] <= 0.90 * ignitionVol {
					washed = true
					break
				}
			}
...
// 🎯 核心升級：強效多重備援爬蟲！先爬 MoneyDJ，遇到 0.00 將其視為假佔位符略過，確保抓出真實發佈的近四季
func fetchQuarterlyEPS(stockID string) ([]string, []float64, error) {
	...
	for _, mirror := range mirrors {
		...
		// 🎯 核心修正 1：整列搜尋防偏位機制！不再依賴 cells[0]，避免被空白或隱藏儲存格干擾
		...
		if len(rawQuarters) > 0 && len(rawEps) > 0 {
			...
			for i := 0; i < minLen; i++ {
				...
				// 🎯 核心防線 2：極致過濾假佔位符！只要遇到 0.00，直接當作「財報未公佈」，強制忽略此季！
			...
			if len(validQuarters) > 0 {
				...
				// 因為前面已經將所有的 0 略過，留下來的 100% 都是真實更新的財報，直接回傳即可！
				return finalQ, finalEps, nil
			}
		}
	}

	// 方案 B: 整個 MoneyDJ 體系全軍覆沒，呼叫 Yahoo Finance JSON API 救火
	yahooAuthMu.Lock()
	crumb := yahooCrumb
	cookie := yahooCookie
	yahooAuthMu.Unlock()
									...
									if quarterly, ok := ec["quarterly"].([]interface{}); ok && len(quarterly) > 0 {
										...
										for _, qItem := range quarterly {
											...
											if okActual {
												...
												// Yahoo 負數修正：若 raw=0 但 fmt="-0.52"，優先採用 fmt
											...
											// 🎯 核心防線 3：Yahoo API 的 actual 如果是 0.00，代表該季未公佈！絕對不要去抓 estimate 假數字，直接略過！
...
// WatchlistItem 定義觀察清單內單筆股票的結構 (儲存於 JSON 檔案的靜態資料)
...
// ✨ 新增：WatchlistStat 定義回傳給前端的即時與歷史動態統計數據
type WatchlistStat struct {
	CurrentPrice     float64 `json:"currentPrice"`
	HighestPrice     float64 `json:"highestPrice"`
	HighestGain      float64 `json:"highestGain"` // 加入觀察後最高漲幅(%)
	ElapsedDays      int     `json:"elapsedDays"` // 經過時間(天數) 
	TotalTradingDays int     `json:"totalTradingDays"` // ✨ 新增：總經過交易日
}
...
// ✨ 新增：GetWatchlistStats 接收完整的觀察清單，回傳包含最新股價、最高漲幅與經過時間的綜合數據
func (a *App) GetWatchlistStats(items []WatchlistItem) map[string]WatchlistStat {
	...
	for _, item := range items {
		...
		go func(stockItem WatchlistItem) {
			...
			if err == nil {
				// 呼叫既有的 Yahoo API 函數，查詢加入日期至今的歷史最高價
				records, errYahoo := downloadYahooFinance(stockItem.ID, addDate.Unix(), now.Unix())
				highestPrice := stockItem.AddPrice // 預設底線為加入時的價格
				
				tradingDaysElapsed := 0 // 總經過交易日
				highestTradingDays := 0 // 創下最高價時的交易日
				
				if errYahoo == nil && len(records) > 1 {
					// 略過第一行標題列
					for i := 1; i < len(records); i++ {
						row := records[i]
						// records 格式: Date, Open, High, Low, Close, Adj Close, Volume
						if len(row) > 4 && row[4] != "null" {
							...
							if errDate == nil {
								...
								if closePrice > highestPrice {
									...
									highestTradingDays = tradingDaysElapsed // 記錄當下是第幾交易日
								}
							}
						}
					}
				...
				// 🎯 分別記錄「創高耗費交易日」與「總經過交易日」
				stat.ElapsedDays = highestTradingDays
				stat.TotalTradingDays = tradingDaysElapsed // ✨ 把迴圈算出來的總交易日傳給前端

				// 計算最高漲幅 (%)
				if stockItem.AddPrice > 0 {
					stat.HighestGain = ((highestPrice - stockItem.AddPrice) / stockItem.AddPrice) * 100
				}
			}
