...
type WatchlistStat struct {
	...
	BasePrice        float64 `json:"basePrice"` // 🎯 新增：校正後的加入日真實收盤價
	HighestPrice     float64 `json:"highestPrice"`
	HighestGain      float64 `json:"highestGain"` // 加入觀察後最高漲幅(%)
	ElapsedDays      int     `json:"elapsedDays"` // 經過時間(天數) 
	TotalTradingDays int     `json:"totalTradingDays"` // 總經過交易日
}
					...
					for i := 1; i < len(records); i++ {
						...
						if len(row) > 4 && row[4] != "null" {
							...
							if errDate == nil {
								...
								// 🎯 核心校正：若 K 線日期等於加入日，強制將該日收盤價作為真實比較基準
								if parsedDate.Equal(addDate) {
									basePrice = closePrice
									// 同步重置最高價底線，避免被舊的錯誤 AddPrice 污染
