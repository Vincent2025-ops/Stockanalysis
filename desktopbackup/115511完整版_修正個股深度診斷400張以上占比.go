...
func httpGetWithRetry(url string) (*http.Response, error) {
	...
	// 🎯 核心防卡頓：強制將逾時壓縮至 4 秒，防止遭 API 阻擋時卡死整個系統超過一分鐘
	client := &http.Client{
	...
	for i := 0; i < 2; i++ {
		...
		time.Sleep(300 * time.Millisecond) // 縮短重試間隔，爭取極速體驗
	}
...
// ensureTDCCData 將 80MB 的集保檔案下載轉移至完全獨立的「非阻塞背景執行緒」
func ensureTDCCData() {
	if !atomic.CompareAndSwapInt32(&tdccDownloadFlag, 0, 1) { return }
	go func() {
		defer atomic.StoreInt32(&tdccDownloadFlag, 0)
		fileName := "tdcc_cache.csv"
		info, errStat := os.Stat(fileName)
		
		if os.IsNotExist(errStat) || time.Since(info.ModTime()) > 24*time.Hour || (errStat == nil && info.Size() < 10000000) {
			client := &http.Client{
			...
			if tdccResp, tdccErr := client.Do(req); tdccErr == nil && tdccResp.StatusCode == 200 {
				defer tdccResp.Body.Close()
				out, errCreate := os.Create(fileName + ".tmp")
				if errCreate == nil {
					copiedBytes, _ := io.Copy(out, tdccResp.Body)
					out.Close()
					if copiedBytes > 10000000 {
...
func fetchLargeShareholders(stockID string) (float64, float64, bool) {
	...
	// 用 Channel 進行 5 線程極速競賽
	resChan := make(chan chipResult, 10)
	
	var wg sync.WaitGroup

	// 1. MoneyDJ (3 個備援站同時併發抓取)
	mirrors := []string{
	...
	for _, m := range mirrors {
		...
		go func(mirror string) {
			...
			resp, err := httpGetWithRetry(fmt.Sprintf("https://%s/z/zc/zck/zck_%s.djhtm", mirror, stockID))
			if err != nil { return }
			defer resp.Body.Close()
			...
			for _, row := range rows {
				...
				// 🎯 無敵滑動引擎 (MoneyDJ 版：完全無視日期與股價)
				// 抽出所有帶有小數點的數字，排除人數等整數欄位
				var decCells []float64
				for _, cell := range cells {
					if strings.Contains(cell, ".") {
						v, err := parseFinancialNumber(cell)
						// 過濾掉 > 100.01 的干擾 (如 1600.00 千元股價)
						if err == nil && v >= 0 && v <= 100.01 {
							decCells = append(decCells, v)
						}
					}
				}
				
				// 確保陣列有足夠長度，最後 4 個必然是 400~1000 以上的級距比例
				if len(decCells) >= 4 {
					p1000 := decCells[len(decCells)-1]
					p800 := decCells[len(decCells)-2]
					p600 := decCells[len(decCells)-3]
					p400 := decCells[len(decCells)-4]
					
					// 暴力加總 4 個級距
					total400 := p1000 + p800 + p600 + p400
					if total400 > 0 && total400 <= 100.01 && p1000 >= 0 {
						resChan <- chipResult{super: p1000, large: total400}
						return
					}
				} else if len(decCells) >= 2 {
					// 簡化版表格的備援判定
					v1000 := decCells[len(decCells)-1]
					v400 := decCells[len(decCells)-2]
					
					if v400 >= v1000 && v400 > 0 && v400 <= 100.01 && v1000 >= 0 {
	...
	go func() {
		defer wg.Done()
		resp, err := httpGetWithRetry(fmt.Sprintf("https://histock.tw/stock/large.aspx?no=%s", stockID))
		if err != nil { return }
		defer resp.Body.Close()
		...
		for _, row := range rows {
			...
			// 🎯 終極連續序列引擎 (HiStock 版：無視週漲跌幅)
			var pctCells []float64
			for _, cell := range cells {
				if strings.Contains(cell, "%") {
					v, err := parseFinancialNumber(strings.ReplaceAll(cell, "%", ""))
					if err == nil {
						pctCells = append(pctCells, v) 
					}
				}
			}
			
			// 大戶籌碼鐵律：400 >= 1000。
			// 倒數第二個必定是「1000張」，倒數第三個必定是「400張」
			if len(pctCells) >= 3 {
				v1000 := pctCells[len(pctCells)-2]
				v400 := pctCells[len(pctCells)-3]
				
				if v400 >= v1000 && v400 <= 100.01 && v1000 >= 0 && v400 > 0 {
					resChan <- chipResult{super: v1000, large: v400}
					return
				}
			}
	...
	go func() {
		...
		if json.Unmarshal(body, &result) == nil && len(result.Data) > 0 {
			...
			for _, item := range result.Data {
				...
				// 🎯 將 12(400~600)、13(600~800)、14(800~1000)、15(1000+) 四個級距全部相加，這就是標準的 >= 400 張總比例
	...
	// 在背景等候所有 Goroutines 結束並關閉 Channel
	go func() {
		wg.Wait()
		close(resChan)
	}()
...
WaitLoop:
	for {
		...
		case res, ok := <-resChan:
			if !ok {
				break WaitLoop // 通道已關閉，所有線上查詢皆失敗，跳出迴圈執行 TDCC 備援
			}
			if res.large > 0 {
				return res.super, res.large, true
			}
		case <-time.After(8500 * time.Millisecond):
			// 延長等待時間至 8.5 秒，確保慢網速情況下也能抓到資料
			break WaitLoop
		}
	}
