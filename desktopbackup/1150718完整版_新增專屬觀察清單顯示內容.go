...
func (a *App) FetchNBreakout() ([]Top10Stock, error) {
	...
	for _, c := range candidates {
		...
		go func(cand stockCandidate) {
			...
			// 🎯 排除已噴出：目前收盤價不可超過起漲高點的 5%，嚴格限制在突破邊緣或仍在箱型內-> 1.05放寬至1.12
			if todayClose > ignitionHigh*1.12 { return }

			// 必須要有洗盤期 (至少表態後經過 2 天)
			if n - 1 - ignitionIdx < 2 { return }

			// 🎯 洗盤確認：表態日之後到昨天之間，必須出現過量縮至表態量 50% 以下的窒息量->放寬至70%
			washed := false
			for i := ignitionIdx + 1; i < n-1; i++ {
				if vols[i] <= 0.70 * ignitionVol {
					washed = true
					break
				}
			}
	...
	// 🎯 新增：溫和的 API 阻擋提醒機制
	if apiFailCount > 0 {
		warningFeature := fmt.Sprintf("⚠️ 系統提示：本次掃描有 %d/%d 檔股票遭 Yahoo API 阻擋未測。若持續無標的，請稍後再試或重啟網路換 IP", apiFailCount, len(candidates))
		
		if len(results) == 0 {
			// 狀況一：完全沒有掃到股票，直接在畫面上顯示一筆假的標的來當作警告
			...
			// 狀況二：有掃到股票，但未滿 10 檔，就把警告接在清單的最下面
			results = append(results, Top10Stock{
				Indicator: "系統提醒", 
				ID: "----", 
				Name: "部分受限", 
