...
interface BasicInfo { pe: string; pb: string; roe: string; eps: string; assessment: string; }
...
declare global {
  interface Window {
    go: { main: { App: {
          ...
          GetCurrentPrices: (ids: string[]) => Promise<Record<string, number>>;
    }; }; };
...
const App: React.FC = () => {
  ...
  const loadWatchlistData = async () => {
    ...
    try {
      ...
      if (list.length > 0) {
        ...
        const prices = await window.go.main.App.GetCurrentPrices(ids);
  ...
  // ✨ 進階升級：加入垂直時間網格、動態 X 軸標籤與互動式 Tooltip (具備完善邊界碰撞防護)
                ...
                <table className="w-full text-left">
                  <thead className="bg-slate-900/80 border-b border-slate-800">
                    <tr>
                      ...
                      <th className="p-5 text-slate-400 uppercase text-sm font-bold tracking-wider">名稱</th>
                      <th className="p-5 text-slate-400 uppercase text-sm font-bold tracking-wider">加入日期</th>
                      ...
                      <th className="p-5 text-slate-400 uppercase text-sm font-bold tracking-wider">至今績效</th>
                  ...
                  <tbody className="divide-y divide-slate-800/50">
                    ...
                    {watchlist.map((item) => {
                      const currentPrice = watchlistPrices[item.id] || 0;
                      const perf = item.addPrice > 0 && currentPrice > 0 ? ((currentPrice - item.addPrice) / item.addPrice) * 100 : 0;
                      ...
                      return (
                        <tr key={item.id} className="hover:bg-slate-800/40 transition-colors group">
                          ...
                          <td className="p-5 font-mono text-xl text-slate-300">${item.addPrice.toFixed(2)}</td>
                          ...
                          <td className={`p-5 font-mono text-xl font-black ${currentPrice > 0 ? perfColor : 'text-slate-500'}`}>
